...
test("vector/badargs", function()
	...
	end, "./gn32/test/vector.lua:%d+: xyFromRadialRad%(rtheta%): bad args %(42%): at argument 1: expected two numbers, or a table containing two numbers")

	assert.error(function()
		vector.xyFromRadialDeg(Entity():setPosition(200, 300))
	end, "./gn32/test/vector.lua:%d+: xyFromRadialDeg%(rtheta%): bad args %(table: 0x[0-9a-f]+%): at argument 1: expected two numbers, or a table containing two numbers")

	assert.error(function()
		vector.radialRadFromXY(true, 2)
	end, "./gn32/test/vector.lua:%d+: radialRadFromXY%(xy%): bad args %(true, 2%): at argument 1: expected two numbers, an entity, or a table containing two numbers")

	assert.error(function()
		vector.radialDegFromXY(1, 2, 3)
	end, "./gn32/test/vector.lua:%d+: radialDegFromXY%(xy%): bad args %(1, 2, 3%): at argument 3: too many arguments")

	assert.error(function()
		local e = Entity()
		e:destroy()
		vector.xyRotateRad(e, 3)
	end, "./gn32/test/vector.lua:%d+: xyRotateRad%(xy, number%): bad args %(table: 0x[0-9a-f]+, 3%): at argument 1: argument 1 is a destroyed entity")

	assert.error(function()
		vector.xyRotateDeg(1, nil, 3)
	end, "./gn32/test/vector.lua:%d+: xyRotateDeg%(xy, number%): bad args %(1, nil, 3%): at argument 1: expected two numbers, an entity, or a table containing two numbers")

	assert.error(function()
		vector.distance(1, 2, 3)
	end, "./gn32/test/vector.lua:%d+: distance%(xy, xy%): bad args %(1, 2, 3%): at argument 3: expected two numbers, an entity, or a table containing two numbers")
end)
